//===--- DoesContainsWorkChecker.cpp - Check for locking problems ---*- C++ -*--===//
//
...
//===----------------------------------------------------------------------===//
//
// This defines DoesContainsWorkChecker, a simple lock -> unlock checker.
...
namespace {
class DoesContainsWorkChecker : public Checker<check::PostStmt<CallExpr> > {
...
void DoesContainsWorkChecker::checkPostStmt(const CallExpr *CE,
...
void DoesContainsWorkChecker::AcquireLock(CheckerContext &C, const CallExpr *CE,
...
void DoesContainsWorkChecker::ReleaseLock(CheckerContext &C, const CallExpr *CE,
                                     SVal lock) const {
    const MemRegion *lockR = lock.getAsRegion();
    if (!lockR) return;
...
void ento::registerDoesContainsWorkChecker(CheckerManager &mgr) {
    mgr.registerChecker<DoesContainsWorkChecker>();
}
